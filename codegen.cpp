    ...
    while (i < s.size()) {
        ...
        // {varname} → placeholder
            ...
            if (end != std::string::npos) {
                ...
                // O tipo exato do placeholder pode ser ajustado dinamicamente depois se necessário, %d por padrão.
                fmt += "%d";
                i = end + 1;
            } else {
                fmt += s[i++];
...
llvm::Value* CodeContext::genExpr(ExprNode* node) {
    ...
    if (auto* n = dynamic_cast<InputExpr*>(node)) {
        llvm::FunctionCallee scanfFunc = module->getOrInsertFunction(
            "scanf",
            llvm::FunctionType::get(builder.getInt32Ty(),
                llvm::PointerType::get(llvm::Type::getInt8Ty(context), 0), true)
        );

        llvm::Value* tempAlloca = builder.CreateAlloca(builder.getInt32Ty(), nullptr, "inputtmp");
        llvm::Value* fmtVal = builder.CreateGlobalStringPtr("%d");
        builder.CreateCall(scanfFunc, {fmtVal, tempAlloca});
        
        return builder.CreateLoad(builder.getInt32Ty(), tempAlloca, "inputval");
    }
    ...
    // Chamada de Expressão / Função (CallExpr)
    if (auto* n = dynamic_cast<CallExpr*>(node)) {
        llvm::Function* callee = module->getFunction(n->name);
        if (!callee) {
            std::cerr << "Erro: Função não declarada no contexto: " << n->name << std::endl;
            return nullptr;
        }

        // Avalia e gera código para todos os argumentos passados
        std::vector<llvm::Value*> argsV;
        for (ExprNode* arg : n->args) {
            llvm::Value* argVal = genExpr(arg);
    ...
    if (auto* n = dynamic_cast<BinaryOp*>(node)) {
        ...
        if (n->op == "%")  return builder.CreateSRem(toI32(l), toI32(r), "mod"); // Adicionado operador de resto
    ...
    if (auto* n = dynamic_cast<TernaryExpr*>(node)) {
        ...
        // Garante dinamicamente que o tipo do nó PHI herda o tipo real retornado pelos ramos
        llvm::Type* phiType = thenVal->getType();
...
void CodeContext::genStmt(StmtNode* node) {
    ...
    if (auto* n = dynamic_cast<DeclStmt*>(node)) {
        ...
        // CORREÇÃO: Obter a função atual e criar um builder apontando para o início do bloco 'entry'
        llvm::Function* fn = builder.GetInsertBlock()->getParent();
        llvm::IRBuilder<> entryBuilder(&fn->getEntryBlock(), fn->getEntryBlock().begin());
        
        // Criar o alloca no bloco de entrada para que execute apenas uma vez
        llvm::Value* alloca = entryBuilder.CreateAlloca(ty, nullptr, n->name);
        ...
        if (n->init) {
            ...
            builder.CreateStore(val, alloca); // O store continua ocorrendo no fluxo normal
        }
        return;
    }
    ...
    if (auto* n = dynamic_cast<PrintStmt*>(node)) {
        ...
        for (auto& varName : vars) {
            ...
            if (it != locals.end()) {
                llvm::Type* targetTy = localTypes.count(varName) ? localTypes[varName] : builder.getInt32Ty();
                llvm::Value* loaded = builder.CreateLoad(targetTy, it->second, varName);
                
                // Conversão dinâmica para printf numérico caso seja booleano
                if (loaded->getType() == builder.getInt1Ty()) {
                    loaded = builder.CreateZExt(loaded, builder.getInt32Ty(), "zext");
                }
    ...
    if (auto* n = dynamic_cast<DoWhileStmt*>(node)) {
        ...
        // CORREÇÃO: Só insere a condição e o desvio se o bloco não foi terminado (ex: por um return)
        if (!builder.GetInsertBlock()->getTerminator()) {
            llvm::Value* cond = genExpr(n->cond);
            // Normaliza condição para i1
            if (cond->getType() != builder.getInt1Ty())
    ...
    if (auto* n = dynamic_cast<ExprStmt*>(node)) {
        genExpr(n->expr); // Avalia a expressão contida (que pode ser um AssignExpr)
        return;
    }
...
void CodeContext::genFunction(FuncDecl* func) {
    ...
    for (auto& arg : fn->args()) {
        ...
        localTypes[p->name] = arg.getType();
    }

    // Gera o corpo
    genStmt(func->body);

    // Se a função não terminou com return, adiciona um padrão seguro
    if (!builder.GetInsertBlock()->getTerminator()) {
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Uso: ./valc arquivo.val" << std::endl;
        return 1;
    }

    std::string filename = argv[1];
    if (filename.find(".val") == std::string::npos) {
    ...
    std::string base = filename.substr(0, filename.find(".val"));
